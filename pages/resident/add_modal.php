...
<div id="addCourseModal" class="modal fade">
    <form class="form-horizontal" method="post" enctype="multipart/form-data">
        ...
        <div class="col-sm-4">
            <input name="txt_lname" class="form-control input-sm typeahead" type="text" placeholder="Lastname" />
        </div>
        <div class="col-sm-4">
            <input name="txt_fname" class="form-control input-sm col-sm-4 typeahead" type="text" placeholder="Firstname" />
        </div>
        <div class="col-sm-4">
            <input name="txt_mname" class="form-control input-sm col-sm-4 typeahead" type="text" placeholder="Middlename" />
        </div>
    </div>
        ...
        $(document).ready(function () {
            ...
            if ($query->num_rows > 0) {
                while ($result = $query->fetch_assoc()) {
                    echo 'fullNames.push("'.addslashes($result['FullName']).'");';
                }
            }
            ?>

            // Initialize Typeahead for full names
            $('input[name="txt_lname"]').typeahead({
                source: fullNames,
                afterSelect: function (item) {
                    // Split the full name to distribute components
                    var parts = item.split(', ');
                    // Update the input fields
                    $('input[name="txt_lname"]').val(parts[0]);
                    $('input[name="txt_fname"]').val(parts[1]);
                    $('input[name="txt_mname"]').val(parts[2]);
                }
            });

            // Initialize Typeahead for first name
            $('input[name="txt_fname"]').typeahead({
                source: fullNames,
                afterSelect: function (item) {
                    // Split the full name to distribute components
                    var parts = item.split(', ');
            ...
            // Initialize Typeahead for middle name
            $('input[name="txt_mname"]').typeahead({
                source: fullNames,
                afterSelect: function (item) {
                    // Split the full name to distribute components
                    var parts = item.split(', ');
...
<script>
    $(document).ready(function () {
        ...
        // Initialize Typeahead for full names
        $('input[name="txt_lname"], input[name="txt_fname"], input[name="txt_mname"]').typeahead({
            source: fullNames,
            afterSelect: function (item) {
                var parts = item.split(', ');
                $('input[name="txt_lname"]').val(parts[0]);
